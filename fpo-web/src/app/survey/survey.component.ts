...
export class SurveyComponent  {
  ...
  complete() {
    if (this.surveyModel.data.FilterCS === 'FilterSmallClaim' || this.surveyModel.data.q5a === 'ssc' || this.surveyModel.data.q5 === 'ssc' ) {
      this.onNavigateToSmallClaim()
    } else if (this.surveyModel.data.FilterCS === 'FilterSupremeCourt' || this.surveyModel.data.q3a === 'yes' || this.surveyModel.data.q4a === 'yes' || this.surveyModel.data.q5 === 'scbc' || this.surveyModel.data.q5a === 'scbc' ) {
      this.onNavigateToSupremeCout();
    } else if (this.surveyModel.data.FilterCS === 'FilterNo') {
      this.surveyModel.completeLastPage();
    }
  }

  onNavigateToSmallClaim(): void {
    let userInfo = localStorage.getItem('userInfo');
    if (this.surveyModel.completeLastPage()) {
      if (userInfo !== '') {
        localStorage.setItem('navUrl', 'smallclaim/survey');
        this.router.navigateByUrl('smallclaim');
      } else {
        this.router.navigateByUrl('prv/status');
      }
    }
  }

  onNavigateToSupremeCout(): void {
    if (this.surveyModel.completeLastPage()) {
      this.router.navigateByUrl('supremecourt')
    }
  }
